...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    this.dragger = function (e) {
        ...
        //this.set && this.set.animate({"fill-opacity": .1}, 200) && this.set.toFront();  // cannot fire click event
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function(node) {
        ...
        shape.click( function() { 
            console.log( shape.getBBox().x + ' ' + shape.getBBox().y );
            clicked? $('#'+node.id).hide() :
            $('#'+node.id).css({
                position: "absolute",
                top: shape.getBBox().y + $('div.content').position().top,
                left: shape.getBBox().x + $('div.content').position().left + 50 }).show();
            clicked = !clicked;
            //( node.url instanceof Array )? window.open( node.url[0] ) : window.open(node.url); 
        } );
        //shape.mouseout( function() { $('#fp').hide(); });
        //shape.mouseover( function() { console.log('aaa'); });
